...
int calculateInitialQuantum(const vector<Process>& processes) {
    ...
    for (const auto& process : processes) {
        ...
        if (process.burstTime > highestBurst) {
            highestBurst = process.burstTime;
        }
    }

    return round((highestBurst + round(totalBurst/processes.size())) / 2);
}
...
void findWaitingTime(vector<Process>& processes, int n, int bt[], int wt[], int& quantum) {
    ...
    while (true) {
        ...
        if (!readyQueue.empty()) {
            ...
            int highestRemainingBurst = 0;
            for (int i = 0; i < n; i++) {
                if (remainingTime[i] > highestRemainingBurst) {
                    highestRemainingBurst = remainingTime[i];
                }
            }


            quantum = round((highestRemainingBurst + round(accumulate(remainingTime, remainingTime + n, 0))) / 2);
